...
export function formatNPR(value: number, compact = false): string {
	...
	if (compact) {
		if (Math.abs(safeValue) >= 1e10)
			return `Rs. ${(safeValue / 1e10).toFixed(2)} Arba`;
		if (Math.abs(safeValue) >= 1e7)
			return `Rs. ${(safeValue / 1e7).toFixed(2)} Cr`;
		if (Math.abs(safeValue) >= 1e5)
			return `Rs. ${(safeValue / 1e5).toFixed(2)} L`;
		if (Math.abs(safeValue) >= 1e3)
			return `Rs. ${(safeValue / 1e3).toFixed(1)}K`;
	}
...
export function calculateCGT(
	profit: number,
	type: "individual_short" | "individual_long" | "institution",
): number {
...
export function weightedAvgPrice(
	transactions: { qty: number; price: number }[],
): number {
 ...
 */
export function getTimeToMarketEvent(): {
	event: string;
	hours: number;
	minutes: number;
	seconds: number;
	...
	const remaining =
		(daysUntilOpen - 1) * secsInDay + (secsInDay - totalSeconds) + openSeconds;
...
export function calcRSI(closes: number[], period = 14): (number | null)[] {
	...
	for (let i = 0; i < closes.length; i++) {
		...
		if (i < period) {
			...
			const avgGain =
				(prevAvgGain * (period - 1) + gains[gains.length - 1]) / period;
			const avgLoss = avgGain === 0 ? 0 : avgGain / (prevRSI / (100 - prevRSI));
			const newAvgLoss =
				(avgLoss * (period - 1) + losses[losses.length - 1]) / period;
...
export function toNepaliNumerals(num: string | number): string {
	...
	return String(num)
		...
		.map((digit) => {
			...
			return !isNaN(d) ? nepaliNumerals[d] : digit;
		})
		.join("");
}

/**
 * Format number as Nepali words: अर्ब (arab), करोड (crore), लाख (lakh), हजार (thousand)
 * Example: 1234567890 → "१ अर्ब २३ करोड ४५ लाख ६७ हजार ८९०"
 */
export function formatNepaliWords(amount: number): string {
	...
	// अर्ब (10 crores = 1 arab) = 1000000000
	const arab = Math.floor(abs / 1000000000);
	if (arab > 0) words.push(`${toNepaliNumerals(arab)} अर्ब`);

	// करोड (crore) = 10000000
	const crore = Math.floor((abs % 1000000000) / 10000000);
	if (crore > 0) words.push(`${toNepaliNumerals(crore)} करोड`);

	// लाख (lakh) = 100000
	const lakh = Math.floor((abs % 10000000) / 100000);
	if (lakh > 0) words.push(`${toNepaliNumerals(lakh)} लाख`);

	// हजार (thousand) = 1000
	const thousand = Math.floor((abs % 100000) / 1000);
	if (thousand > 0) words.push(`${toNepaliNumerals(thousand)} हजार`);

	// Remaining
	const remaining = abs % 1000;
	if (remaining > 0) words.push(toNepaliNumerals(remaining));
